...
pytestmark = pytest.mark.skipif(not torch.cuda.is_available(), reason="requires a CUDA GPU (OptiX tracer)")
...
def _make_batch(h=48, w=48, device="cuda"):
    ...
    rays_dir = torch.nn.functional.normalize(torch.stack([xs, ys, torch.ones_like(xs)], dim=-1), dim=-1)[None].to(
        device
    )
...
def test_flags_match_traced_graph(primitive_type):
    ...
    assert proc.returncode == 0, f"probe exited {proc.returncode}\nstdout:\n{proc.stdout}\nstderr:\n{proc.stderr}"
