...
struct MoGTracingParams
{
    ...
    PackedTensorAccessor32<float, 2> mogWeightSum; ///< output (only in AH pipeline) sum of all weights gaussian contributed during render pass
...
struct MoGTracingBwdParams
{
    ...
    PackedTensorAccessor32<float, 4> rayError;  ///< integrated error on each ray (NOT A GRADIENT)
    ...
    PackedTensorAccessor32<float, 2> mogErrorBack; ///< output gaussians error times weight
    OptixTraversableHandle handle;
