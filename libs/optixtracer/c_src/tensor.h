...
struct Tensor
{
    ...
    __device__ inline int fetchInt(unsigned int idx) const { return ((int*)val)[idx]; }
   ...
   // Fetch, use broadcasting for tensor dimensions of size 1
    __device__ inline float fetch(unsigned int x, unsigned int y, unsigned int z, unsigned int channel) const
    {
        return fetch(nhwcIndex(z, y, x, channel));
    }
    ...
    __device__ inline void store(unsigned int x, unsigned int y, unsigned int z, unsigned int channel, float _val)
    {
        store(_nhwcIndex(z, y, x, channel), _val);
    }
