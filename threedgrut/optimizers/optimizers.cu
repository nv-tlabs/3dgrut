...
//
//
// Selective Adam implementation was adpoted from gSplat library
// (https://github.com/nerfstudio-project/gsplat/blob/main/gsplat/cuda/csrc/AdamCUDA.cu),
// which is based on the original implementation
// https://github.com/humansensinglab/taming-3dgs that uderlines the work
//
// Taming 3DGS: High-Quality Radiance Fields with Limited Resources by
// Saswat Subhajyoti Mallick*, Rahul Goel*, Bernhard Kerbl, Francisco Vicente
// Carrasco, Markus Steinberger and Fernando De La Torre
//
...
__global__ void selective_adam_update_kernel(
    scalar_t *__restrict__ param, const scalar_t *__restrict__ param_grad,
    scalar_t *__restrict__ exp_avg, scalar_t *__restrict__ exp_avg_sq,
    const bool *__restrict__ visibility, const float lr, const float b1,
    const float b2, const float eps, const uint32_t N, const uint32_t M) {
...
void selective_adam_update_launch(at::Tensor &param,            // [N, ...]
                                  const at::Tensor &param_grad, // [N, ...]
                                  at::Tensor &exp_avg,          // [N, ...]
                                  at::Tensor &exp_avg_sq,       // [N, ...]
                                  const at::Tensor &visibility, // [N]
                                  const float lr, const float b1,
