...
class MixtureOfGaussians(torch.nn.Module):
    ...
    def __getitem__(self, idx):
        ...
        sliced.rotation = torch.nn.Parameter(sliced.rotations[idx])
        sliced.scale = torch.nn.Parameter(sliced.scales[idx])
        sliced.density = torch.nn.Parameter(sliced.densitys[idx])
        sliced.features_albedo = torch.nn.Parameter(sliced.features_albedos[idx])
        sliced.features_specular = torch.nn.Parameter(sliced.features_speculars[idx])
        return sliced
