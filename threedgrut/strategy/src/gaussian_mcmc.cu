...
//
// MCMC implementation was adpoted from gSplat library
// (https://github.com/nerfstudio-project/gsplat/blob/main/gsplat/strategy/mcmc.py),
// which is based on the original implementation
// https://github.com/ubc-vision/3dgs-mcmc that uderlines the work
//
...
// Hossam Isack, Abhishek Kar, Andrea Tagliasacchi and Kwang Moo Yie
//
...
__global__ void compute_relocation_kernel(
    const int N, const float *__restrict__ opacities,
    const float *__restrict__ scales, const int *__restrict__ ratios,
    const float *__restrict__ binoms, const int n_max,
    float *__restrict__ new_opacities, float *__restrict__ new_scales) {
...
std::tuple<torch::Tensor, torch::Tensor> compute_relocation_tensor_cu(
    ...
    const torch::Tensor ratios, const torch::Tensor binoms,
  ...
  if (num_gaussians) {
    compute_relocation_kernel<<<blocks, threads, 0, stream>>>(
        num_gaussians, opacities.data_ptr<float>(), scales.data_ptr<float>(),
        ratios.data_ptr<int>(), binoms.data_ptr<float>(), max_num_gaussians,
