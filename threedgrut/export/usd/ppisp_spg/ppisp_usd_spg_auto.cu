...
static __device__ __forceinline__ float applyCRF(
    ...
    const float center   = sigmoid(centerRaw);

    const float lerpVal = (shoulder - toe) * center + toe;
...
extern "C" __global__ void ppispProcessAuto(
    ...
    const float exposureOffset      = controllerParams[0];
    const float2 colorLatentBlue    = makeFloat2(controllerParams[1], controllerParams[2]);
    const float2 colorLatentRed     = makeFloat2(controllerParams[3], controllerParams[4]);
    const float2 colorLatentGreen   = makeFloat2(controllerParams[5], controllerParams[6]);
    const float2 colorLatentNeutral = makeFloat2(controllerParams[7], controllerParams[8]);
    const float3 rgb                = applyPPISPColor(
        ...
        params[0],
        exposureOffset,
        makeFloat2(params[1], params[2]),
        params[3],
        params[4],
        params[5],
        makeFloat2(params[6], params[7]),
        params[8],
        params[9],
        params[10],
        makeFloat2(params[11], params[12]),
        params[13],
        params[14],
        params[15],
        colorLatentBlue,
