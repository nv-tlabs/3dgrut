...
static __device__ __forceinline__ void conv1Forward(float r, float g, float b, const float* __restrict__ weights, float feat[16]) {
#pragma unroll
    for (int o = 0; o < 16; ++o) {
...
static __device__ __forceinline__ void conv2Forward(const float fin[16], const float* __restrict__ weights, float fout[32]) {
#pragma unroll
    for (int o = 0; o < 32; ++o) {
...
static __device__ __forceinline__ void conv3ForwardChunk(
    const float fin[32],
    const float* __restrict__ weights,
    int firstChannel,
    float fout[CNN_FEATURE_CHUNK]) {
#pragma unroll
...
static __device__ __forceinline__ void cnnForwardAtDownsampledPixelChunk(
    ...
    const int x1 = min(x0 + INPUT_DOWNSAMPLING, inW);
    const int y1 = min(y0 + INPUT_DOWNSAMPLING, inH);
...
extern "C" __global__ void controllerPoolProcess(
    ...
    const int dsW = max(1, inW / INPUT_DOWNSAMPLING);
    const int dsH = max(1, inH / INPUT_DOWNSAMPLING);
    ...
    //     outControllerFeatures[c * POOL_CELL_COUNT + cell].
    ...
    for (int firstChannel = 0; firstChannel < CNN_FEATURE_DIM; firstChannel += CNN_FEATURE_CHUNK) {
...
#pragma unroll
        ...
        for (int idx = tid; idx < count; idx += POOL_THREAD_GROUP_SIZE) {
            ...
            float featChunk[CNN_FEATURE_CHUNK];
            cnnForwardAtDownsampledPixelChunk(
                inW, inH, dx, dy, inHdrColor, weights, firstChannel, featChunk);
...
#pragma unroll
        for (int c = 0; c < CNN_FEATURE_CHUNK; ++c) {
            ...
            if (tid == 0) {
                ...
                outControllerFeatures[channel * POOL_CELL_COUNT + cell] = gsReduce[0] * invCount;
            }
            __syncthreads();
        }
    }
...
extern "C" __global__ void controllerProcess(
    ...
    for (int o = tid; o < MLP_HIDDEN_DIM; o += MLP_THREAD_GROUP_SIZE) {
        ...
        for (int i = 0; i < POOL_FEATURE_LEN; ++i) {
            v += controllerFeatures[i] * weights[OFF_TRUNK0_W + o * MLP_INPUT_DIM + i];
        }
        v += priorExposure * weights[OFF_TRUNK0_W + o * MLP_INPUT_DIM + POOL_FEATURE_LEN];
