...
// Tile-local UV for a tiled RenderProduct atlas. SPG runs once on the full
// atlas; vignetting must be evaluated relative to each tile (same camera
// rendered through different poses), not the whole atlas. For an untiled
// product (tileCountX == tileCountY == 1) this reduces exactly to the
// full-image UV used before tiling support was added.
...
extern "C" __global__ void ppispProcess(int width, int height, int tileCountX,
                                        int tileCountY,
  ...
  const float3 rgb = applyPPISPColor(
      makeFloat3(pixel.x, pixel.y, pixel.z), uv, params[0], params[1],
      makeFloat2(params[2], params[3]), params[4], params[5], params[6],
      makeFloat2(params[7], params[8]), params[9], params[10], params[11],
      makeFloat2(params[12], params[13]), params[14], params[15], params[16],
      makeFloat2(params[17], params[18]), makeFloat2(params[19], params[20]),
      makeFloat2(params[21], params[22]), makeFloat2(params[23], params[24]),
