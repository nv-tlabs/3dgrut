...
__global__ void computeMeshFaceBufferKernel(const uint32_t fNum,
...
void computeMeshFaceBuffer(uint32_t fNum, const float3 *verts,
                           const int3 *faces, float3 *fPrimVrt, int3 *fPrimTri,
                           cudaStream_t stream) {
  const uint32_t threads = 1024;
  const uint32_t blocks = div_round_up(static_cast<uint32_t>(fNum), threads);

  computeMeshFaceBufferKernel<<<blocks, threads, 0, stream>>>(
      fNum, verts, faces, fPrimVrt, fPrimTri);
}
