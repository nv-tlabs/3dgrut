...
template <class T> static __device__ __inline__ T clamp(T x, T _min, T _max) {
  return min(_max, max(_min, x));
}
static __device__ inline float3 clampf3(const float3 &a, const float _min,
                                        const float _max) {
  return make_float3(clamp(a.x, _min, _max), clamp(a.y, _min, _max),
                     clamp(a.z, _min, _max));
}
...
static __device__ inline void bwd_dot(float3 a, float3 b, float3 &d_a,
                                      float3 &d_b, float d_out) {
  d_a.x += d_out * b.x;
  d_a.y += d_out * b.y;
  d_a.z += d_out * b.z;
  d_b.x += d_out * a.x;
...
static __device__ inline float sum(const float3 v) { return v.x + v.y + v.z; }
...
static __device__ inline void bwd_cross(float3 a, float3 b, float3 &d_a,
                                        float3 &d_b, float3 d_out) {
...
__device__ static inline void bwdCross3(const float3 &v0, const float3 &v1,
                                        const float3 &v2, const float3 &grad,
                                        float3 &g_v0, float3 &g_v1,
                                        float3 &g_v2) {
  float3 a = make_float3(v2.x - v0.x, v2.y - v0.y, v2.z - v0.z);
  float3 b = make_float3(v1.x - v0.x, v1.y - v0.y, v1.z - v0.z);
...
static __device__ inline float3 reflect(float3 x, float3 n) {
  float3 r = n * 2.0f * dot(n, x);
  return make_float3(r.x - x.x, r.y - x.y, r.z - x.z);
}

static __device__ inline void bwd_reflect(float3 x, float3 n, float3 &d_x,
                                          float3 &d_n, float3 d_out) {
  d_x.x += d_out.x * (2 * n.x * n.x - 1) + d_out.y * (2 * n.x * n.y) +
           d_out.z * (2 * n.x * n.z);
  d_x.y += d_out.x * (2 * n.x * n.y) + d_out.y * (2 * n.y * n.y - 1) +
           d_out.z * (2 * n.y * n.z);
  d_x.z += d_out.x * (2 * n.x * n.z) + d_out.y * (2 * n.y * n.z) +
           d_out.z * (2 * n.z * n.z - 1);

  d_n.x += d_out.x * (2 * (2 * n.x * x.x + n.y * x.y + n.z * x.z)) +
           d_out.y * (2 * n.y * x.x) + d_out.z * (2 * n.z * x.x);
  d_n.y += d_out.x * (2 * n.x * x.y) +
           d_out.y * (2 * (n.x * x.x + 2 * n.y * x.y + n.z * x.z)) +
           d_out.z * (2 * n.z * x.y);
  d_n.z += d_out.x * (2 * n.x * x.z) + d_out.y * (2 * n.y * x.z) +
           d_out.z * (2 * (n.x * x.x + n.y * x.y + 2 * n.z * x.z));
}
...
static __device__ inline void bwd_safe_normalize(const float3 v, float3 &d_v,
                                                 float3 d_out) {
...
// static __device__ inline float3 safe_normalize_bw(const float3& v, const
// float3& d_out)
// {
//     const float l = v.x * v.x + v.y * v.y + v.z * v.z;
//     if (l > 0.0f)
...
//         return make_float3((d_out.x * (v.y * v.y + v.z * v.z) - d_out.y *
//         (v.x * v.y) - d_out.z * (v.x * v.z)),
//                            (d_out.y * (v.x * v.x + v.z * v.z) - d_out.x *
//                            (v.y * v.x) - d_out.z * (v.y * v.z)), (d_out.z *
//                            (v.x * v.x + v.y * v.y) - d_out.x * (v.z * v.x) -
//                            d_out.y * (v.z * v.y))) * il3;
//     }
//     return make_float3(0);
// }

static __device__ inline float3 safe_normalize_bw(const float3 &v,
                                                  const float3 &d_out) {
  const float l = v.x * v.x + v.y * v.y + v.z * v.z;
  if (l > 0.0f) {
    const float il = rsqrtf(l);
...
static __device__ __inline__ float sqr(const float x) { return x * x; }
...
static __device__ inline void branchlessONB(const float3 &n, float3 &b1,
                                            float3 &b2) {
...
static __device__ inline float3 operator*(const float33 &m, const float3 &p) {
  return make_float3(dot(make_float3(m[0].x, m[1].x, m[2].x), p),
...
static __device__ inline float3 matmul_bw_vec(const float33 &m,
                                              const float3 &gdt) {
  return make_float3(gdt.x * m[0].x + gdt.y * m[1].x + gdt.z * m[2].x,
                     gdt.x * m[0].y + gdt.y * m[1].y + gdt.z * m[2].y,
                     gdt.x * m[0].z + gdt.y * m[1].z + gdt.z * m[2].z);
}

static __device__ inline float4 matmul_bw_quat(const float3 &p, const float3 &g,
                                               const float4 &q) {
  float33 dmat;
  dmat[0] = g.x * p;
  dmat[1] = g.y * p;
  ...
  // m[2] = make_float3(2.f * (x * z + r * y), 2.f * (y * z - r * x), (1.f - 2.f
  // * (x * x + y * y)));
...
static __device__ inline void quaternionWXYZToMatrixTranspose(const float4 &q,
                                                              float33 &ret) {
  const float r = q.x;
  const float x = q.y;
  const float y = q.z;
  ...
  ret[0] = make_float3((1.f - 2.f * (y * y + z * z)), 2.f * (x * y - r * z),
                       2.f * (x * z + r * y));
  ret[1] = make_float3(2.f * (x * y + r * z), (1.f - 2.f * (x * x + z * z)),
                       2.f * (y * z - r * x));
  ret[2] = make_float3(2.f * (x * z - r * y), 2.f * (y * z + r * x),
                       (1.f - 2.f * (x * x + y * y)));
}
