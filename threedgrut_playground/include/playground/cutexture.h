...
template <typename T, int C = 1> class CudaTexture2DFloatObject final {
...
public:
  ...
  CudaTexture2DFloatObject(const T *hostData, const int height,
                           const int width) {
  ...
  void reset(const T *hostData, const int height, const int width) {
    ...
    cudaChannelFormatDesc channelDesc = cudaCreateChannelDesc(
        nBytes, (C > 1 ? nBytes : 0), (C > 2 ? nBytes : 0),
        (C > 3 ? nBytes : 0), cudaChannelFormatKindFloat);
    cudaMallocArray(&_array, &channelDesc, width, height);

    cudaMemcpy2DToArray(_array, 0, 0, hostData, width * sizeof(T) * C,
                        width * sizeof(T) * C, height,
