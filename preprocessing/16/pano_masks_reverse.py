...
def run_mask_reconstruction(args):
    ...
    if args.num_cameras == -1:
        num_cameras = 16# ablation auto_detect_num_cameras(input_base_dir)
        if num_cameras == 0:
    ...
    logging.info(f"Output panorama size: {pano_size[0]}x{pano_size[1]}")
    ...
    for mask_name in tqdm(mask_file_names, desc="Reconstructing panorama masks"):
        ...
        # Reconstruct panorama mask
        reconstructed_mask = reconstruct_panorama_mask_from_cameras(
            virtual_masks, cams_from_pano_rotation, pano_size, camera
        )
