...
def evaluate_rays(dense_hit_gIds, rays_o, rays_d, gpos, grot, gscl, gdns, gsh):
    """
    """
    ...
    hit_rays_o = hit_rays_o_dense[dense_hit_mask3]
    hit_rays_d_dense = rays_d[:,None,:].expand(N,D,3)
    hit_rays_d = hit_rays_d_dense[dense_hit_mask3]
    ...
    hit_grad, hit_galpha, hit_gdist = evaluate_gaussians(hit_rays_o, hit_rays_d, hit_gpos, hit_grot, hit_gscl, hit_gdns, hit_gsh)

    ## Broadcast the results back to being dense
    dense_grad = torch.zeros((N,D,3), dtype=dtype, device=device)
    dense_grad[dense_hit_mask3] = hit_grad

    dense_alphas = torch.zeros((N,D), dtype=dtype, device=device)
    dense_alphas[dense_hit_mask] = hit_galpha

    dense_gdist = torch.zeros((N,D), dtype=dtype, device=device)
    dense_gdist[dense_hit_mask] = hit_gdist

    ## Compute transmittance along the ray
    dense_unalpha = torch.cat((torch.ones_like(dense_alphas[:,0:1]), 1. - dense_alphas), dim=-1)
    dense_transmit = torch.cumprod(dense_unalpha, dim=-1)[:,1:]

    ## Add up weighted contributions from each hit
    dense_weight = dense_transmit * dense_alphas
    ray_rad = torch.sum(dense_weight[:None] * dense_grad, dim=-1)
    ray_opacity = dense_transmit[:,-1]
    ray_dist = torch.sum(dense_weight*hit_gdist, dim=-1) # TODO better to use max dist?
    ray_ohit = torch.sum(dense_hit_mask, dim=-1)

    ## Reshape to original size
    ray_rad = ray_rad.reshape(*orig_shape,3)
    ray_opacity = ray_opacity.reshape(*orig_shape)
    ray_dist = ray_dist.reshape(*orig_shape)
    ray_ohit = ray_ohit.reshape(*orig_shape)

    return ray_rad, ray_opacity, ray_ohit, ray_dist

def evaluate_gaussians(rays_o, rays_d, gpos, grot, gscl, gdns, gsh):
    """
    ...
    gposc = rays_o - gpos
    gdist = torch.linalg.norm()
    gposcr = gposc @ grotMat
    gro = giscl * gposcr
    rayDirR = rays_d @ grotMat
    grdu = giscl * rayDirR
    grd = safe_normalize(grdu)
    gcrod = torch.cross(grd, gro, dim=-1)
